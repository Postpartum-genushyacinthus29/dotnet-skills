...
A community-driven catalog of reusable agent skills for .NET development.

Skills cover ASP.NET Core, Blazor, MAUI, WPF, Entity Framework, Orleans, Semantic Kernel, Azure Functions, and more. Install them into Claude, Copilot, Gemini, or Codex to make your AI coding agent smarter about .NET.

**Missing a skill? [Add it!](CONTRIBUTING.md)** This catalog grows when library authors and .NET developers contribute their knowledge.
...
Catalog releases are published automatically from `main` when `skills/` or catalog-generation inputs change. The tool reads the newest non-draft `catalog-v*` release by default, and `--catalog-version` is only for intentional pinning.
...
├── scripts/          # optional
├── references/       # optional
└── agents/           # optional OpenAI adapter
    └── openai.yaml
```
...
| --- | --- | --- | --- |
| `dotnet` | `1.0.0` | Primary router skill for broad .NET work. Classify the repo by app model and cross-cutting concern first, then switch to the narrowest matching .NET skill instead of staying at a generic layer. | [`skills/dotnet/`](skills/dotnet/) |
...
| `dotnet-mvvm` | `1.0.0` | Implement the Model-View-ViewModel pattern in .NET applications with proper separation of concerns, data binding, commands, and testable ViewModels using MVVM Toolkit. | [`skills/dotnet-mvvm/`](skills/dotnet-mvvm/) |
| `dotnet-uno-platform` | `1.0.0` | Build cross-platform .NET applications with Uno Platform targeting WebAssembly, iOS, Android, macOS, Linux, and Windows from a single XAML/C# codebase. | [`skills/dotnet-uno-platform/`](skills/dotnet-uno-platform/) |
...
| Skill | Version | Description | Folder |
| --- | --- | --- | --- |
| `dotnet-mcp` | `1.0.0` | Implement Model Context Protocol (MCP) servers and clients in .NET to enable secure, standardized communication between LLM applications and external tools or data sources. | [`skills/dotnet-mcp/`](skills/dotnet-mcp/) |
